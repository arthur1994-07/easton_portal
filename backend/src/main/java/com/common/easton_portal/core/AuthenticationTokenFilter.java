...
import com.common.core.web.security.base.CertificateParser;
...
public class AuthenticationTokenFilter extends OncePerRequestFilter {
    ...
    private void checkAuthentication(HttpServletRequest request) {
        var token = HttpJwtHelper.getToken(new HttpJwtHelper.RequestService() {
            @Override public String header(String key) {
                return request.getHeader(key);
            }
            @Override public String cookie(String key) {
        ...
        CertificateParser.Data uncheckedData = null;
        try {
            uncheckedData = JwtParser.deserialize(token);
        } catch (Exception ignored) {}
        if (uncheckedData == null) return;
